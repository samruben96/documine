...
**Root Cause:** The `logAuditEvent()` function in `src/lib/admin/audit-logger.ts` was using `createClient()` (regular user-authenticated Supabase client) instead of `createServiceClient()` (service role client).

The RLS policy on `agency_audit_logs` table:
- SELECT: Admins can view audit logs
- INSERT: **No policy exists** for regular users

Since audit logs need to be written by any authenticated user performing actions (not just admins), the regular client fails RLS checks on INSERT.
...
**Why This Is Safe:**
1. Audit logs are append-only - no user data exposure risk from INSERT
2. The function validates `agencyId` and `userId` are provided
3. Query functions (`queryAuditLogs`, `exportAuditLogs`) still use regular client with RLS to enforce admin-only viewing
4. Service client only used for writing, not reading sensitive data

**Files Changed:**
- `src/lib/admin/audit-logger.ts` - Import `createServiceClient`, use it in `logAuditEvent()`

**Key Learning:** For append-only audit/logging tables where:
...
**Pattern:**
```typescript
// Writing audit logs - use service client (any user can create)
export async function logAuditEvent(input: AuditLogInput): Promise<void> {
  const supabase = createServiceClient();
  await supabase.from('audit_logs').insert({...});
}

// Reading audit logs - use regular client (admin-only via RLS)
export async function queryAuditLogs(params: {...}): Promise<AuditLogEntry[]> {
  const supabase = await createClient();
...
The comment "Will be populated by frontend from parsed data" was never implemented - no code existed to populate chart data.
...
**Key Learning:** When AI generates chart configurations (xKey, yKey, chart type), the backend must transform the raw data into chart-ready format. Don't rely on frontend to do data aggregation - it has performance implications and the data is already available server-side.
