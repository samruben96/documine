...
No epics with status "backlog" found in sprint-status.yaml.
    ...
    <action>Resolve output file path using workflow variables.</action>
    <action>Create epic folder: mkdir -p {epic_folder}/stories</action>
    <action>Initialize tech-spec.md by writing the template to {default_output_file}.</action>
  </step>
...
Continuing to regenerate tech spec...
  ...
  <step n="9" goal="Generate epic overview file">
    <action>Create {epic_overview_file} (epic.md) with the following structure:</action>
    <template-output file="{epic_overview_file}">
# Epic {{epic_id}}: {{epic_title}}

**Status:** Contexted
**FRs Covered:** {{frs_covered}}
**Stories:** {{story_count}}
...
{{epic_summary}} (1-2 paragraphs summarizing the epic's purpose)

## Objectives

{{objectives_bullets}} (3-5 key objectives)

## Stories

| Story | Title | Description |
|-------|-------|-------------|
{{stories_table}} (list all stories from sprint-status or PRD)

## Acceptance Summary

{{acceptance_summary}} (bullet points of key acceptance criteria)

## Dependencies

{{epic_dependencies}} (list prerequisite epics or external dependencies)

## Related Documents

- [Tech Spec](./tech-spec.md) - Detailed technical specification
- Link to PRD, Architecture, UX Design as appropriate
    </template-output>
  </step>
  ...
  <step n="10" goal="Validate and mark epic contexted" tag="sprint-status">
    ...
    <action>Add comment "# Location: {epic_folder}/" above the epic entry</action>
    ...
    <output>**✅ Epic Contexted Successfully, {user_name}!**

**Epic Details:**
- Epic ID: {{epic_id}}
- Epic Title: {{epic_title}}
- Location: {epic_folder}/
- Epic Status: contexted (was backlog)

**Generated Files:**
- `{epic_folder}/epic.md` - Epic overview
- `{epic_folder}/tech-spec.md` - Technical specification
- `{epic_folder}/stories/` - Ready for story files
