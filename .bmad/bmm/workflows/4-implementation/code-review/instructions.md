...
Review was saved to story file, but sprint-status.yaml may be out of sync.
  ...
  <step n="8.5" goal="Push approved changes to GitHub" tag="git-push">
    <check if="ad_hoc_review_mode == true">
      <action>Skip git push (ad-hoc review mode)</action>
    </check>

    <check if="ad_hoc_review_mode != true AND {{outcome}} == 'Approve'">
      <critical>Only push to GitHub when review outcome is APPROVE and story is marked done</critical>

      <action>Check git status to see all changes since last commit</action>
      <action>Stage all relevant changes:
        - Story file with review appended
        - Sprint status file
        - All implementation files from Dev Agent Record → File List
        - Any test files
        - Package.json and config changes
      </action>

      <action>Create commit with message format:
        ```
        feat(story-{{epic_num}}.{{story_num}}): {{story_title}}

        Implements story {{epic_num}}.{{story_num}} with all acceptance criteria satisfied.

        Changes:
        - [List key files/features implemented]

        Review: APPROVED
        - All {{ac_count}} acceptance criteria verified
        - All {{task_count}} tasks verified complete
        - Tests passing
      ...
      <action>Push to remote origin on current branch</action>

      <check if="push successful">
        <output>✅ Changes pushed to GitHub successfully</output>
      </check>

      <check if="push failed">
        <output>⚠️ Failed to push to GitHub. Error: {{git_error}}

Please manually push changes with:
```
git add -A
git commit -m "feat(story-{{epic_num}}.{{story_num}}): {{story_title}}"
git push
```
        </output>
      </check>
    ...
    <check if="ad_hoc_review_mode != true AND {{outcome}} != 'Approve'">
      <action>Skip git push (review not approved)</action>
      <output>📋 Git push skipped - review outcome was {{outcome}}, not Approve</output>
    </check>
  </step>
...
- GitHub: {{git_push_status}}
- Action Items: {{action_item_count}}

**Next Steps:**
1. Story is marked done and pushed to GitHub
2. Continue with next story via `*create-story` or `*story-context`
3. Run `*workflow-status` to see current sprint state
      </output>
    </check>

    <check if="ad_hoc_review_mode != true AND {{outcome}} != 'Approve'">
      <output>**⚠️ Story Review Complete, {user_name}!**
...
- Review Outcome: {{outcome}}
- Sprint Status: {{target_status}}
- GitHub: Not pushed (review not approved)
- Action Items: {{action_item_count}}

**Next Steps:**
1. Review the Senior Developer Review notes appended to story
2. Address action items and re-run `*dev-story`
3. Once fixed, re-run `*code-review`
      </output>
    </check>
  </step>
