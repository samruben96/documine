 ...
 * - Creates a processing_job record with status='pending' and agency_id
 * - Returns immediately WITHOUT triggering Edge Function
 * - pg_cron will pick up pending jobs every minute and invoke Edge Function
 *
...
export async function createProcessingJob(
  ...
  // First, get the document details (agency_id needed for job record)
  ...
  // Story 11.1 (AC-11.1.2): Create job with agency_id, status='pending'
  // pg_cron will pick this up within 1 minute and invoke Edge Function
  const { data, error } = await serviceClient
    .from('processing_jobs')
    .insert({
      document_id: documentId,
      agency_id: document.agency_id, // Story 11.1: Include agency_id for RLS
      status: 'pending',
      stage: 'queued',
      progress_percent: 0,
      retry_count: 0,
  ...
  // Story 11.1 (AC-11.1.2): NO synchronous Edge Function trigger
  // pg_cron picks up pending jobs every minute via process_next_document_job()
  log.info('Processing job created (async)', {
    jobId: data.id,
    documentId,
